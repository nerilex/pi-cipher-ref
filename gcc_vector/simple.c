...
void generate_testvectors(size_t key_len, size_t npub_len) {
...
#if P_MOD
	uint8_t ad[PI_PT_BLOCK_LENGTH_BYTES / 2];
	uint8_t msg[7 * 3 * PI_PT_BLOCK_LENGTH_BYTES];
	const int msg_inc = 17;
#else
	...
	for (msg_len = 0; msg_len <= sizeof(msg); msg_len += msg_inc) {
...
int main(void) {
	...
	uint8_t cipher[PI_CT_BLOCK_LENGTH_BYTES * 4 + 100];
	size_t clen;

	PI_ENCRYPT_SIMPLE(cipher, &clen, msg, sizeof(msg), ad, 0, NULL, nonce, 4, key, 16);
	hexdump_block(cipher, clen, 4, 8);
	putchar('\n');
	return 0;
}
